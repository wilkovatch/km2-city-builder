...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
        ...
        public KeyCode translationSnapping2 = KeyCode.RightControl;
        public KeyCode AddSelection2 = KeyCode.RightShift;
        public KeyCode RemoveSelection2 = KeyCode.RightAlt;
		...
		IEnumerator TransformSelected(TransformType transType)
		{
			...
			while(!Input.GetMouseButtonUp(0))
			{
				...
				bool isSnapping = Input.GetKey(translationSnapping) || Input.GetKey(translationSnapping2);
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					if(transType == TransformType.Move)
					{
						...
						if(isSnapping && movementSnap > 0)
						{
							...
							if(hasTranslatingAxisPlane)
							{
								...
								float positionInAxis1 = ExtVector3.MagnitudeInDirection(targetRootsOrdered[0].position, otherAxis1);
								float positionInAxis2 = ExtVector3.MagnitudeInDirection(targetRootsOrdered[0].position, otherAxis2);

								float remainder1;
								float snapAmount1 = CalculateSnapAmount(positionInAxis1, movementSnap, amountInAxis1, out remainder1);
								float remainder2;
								float snapAmount2 = CalculateSnapAmount(positionInAxis2, movementSnap, amountInAxis2, out remainder2);
							...
							{
								float remainder;
								float positionInAxis = Mathf.Abs(ExtVector3.MagnitudeInDirection(targetRootsOrdered[0].position, axis));
								float snapAmount = CalculateSnapAmount(positionInAxis, movementSnap, currentSnapMovementAmount.magnitude, out remainder);
					...
					else if(transType == TransformType.Scale)
					{
						...
						if(isSnapping && scaleSnap > 0)
						{
                            ...
                            var trueSnapToGrid = snapToGrid;
                            snapToGrid = false;
                            float snapAmount = CalculateSnapAmount(targetRootsOrdered[0].localScale.magnitude, scaleSnap, currentSnapScaleAmount, out remainder);
                            snapToGrid = trueSnapToGrid;
					...
					else if(transType == TransformType.Rotate)
					{
						...
						if(isSnapping && rotationSnap > 0)
						{
                            ...
                            var trueSnapToGrid = snapToGrid;
                            snapToGrid = false;
							float snapAmount = CalculateSnapAmount(0, rotationSnap, currentSnapRotationAmount, out remainder);
                            snapToGrid = trueSnapToGrid;
		...
		float CalculateSnapAmount(float curValue, float snapValue, float currentAmount, out float remainder)
		{
			remainder = 0;
			...
			if (snapToGrid)
            {
				float sumValue = curValue + currentAmount;
				return snapValue * (Mathf.Sign(sumValue) * Mathf.Floor(Mathf.Abs(sumValue) / snapValue)) - curValue;
			}
			else if(currentAmountAbs > snapValue)
			{
		...
		void GetTarget()
		{
			...
			if(nearAxis == Axis.None && Input.GetMouseButtonDown(0))
			{
				bool isAdding = Input.GetKey(AddSelection) || Input.GetKey(AddSelection2);
				bool isRemoving = Input.GetKey(RemoveSelection) || Input.GetKey(RemoveSelection2);

				RaycastHit hitInfo; 
				if(Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask))
				{
					Transform target = hitInfo.transform;
                    if (target.gameObject.layer == selectOnlyMask) {
