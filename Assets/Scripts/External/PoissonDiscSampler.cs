...
/// Modified by Wilhelm Kovatch (to make it deterministic)
public class PoissonDiscSampler
{
    ...
    private System.Random rnd = new System.Random(0);
    float RandomValue() {
    ...
    public IEnumerable<Vector2> Samples()
    {
        ...
        yield return AddSample(new Vector2(RandomValue() * rect.width, RandomValue() * rect.height));

        while (activeSamples.Count > 0) {

            // Pick a random active sample
            int i = (int)RandomValue() * activeSamples.Count;
            ...
            for (int j = 0; j < k; ++j) {
                ...
                float angle = 2 * Mathf.PI * RandomValue();
                float r = Mathf.Sqrt(RandomValue() * 3 * radius2 + radius2); // See: http://stackoverflow.com/questions/9048095/create-random-number-within-an-annulus/9048443#9048443
